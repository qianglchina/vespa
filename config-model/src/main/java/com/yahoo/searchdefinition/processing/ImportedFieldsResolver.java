...
public class ImportedFieldsResolver extends Processor {
    ...
    private void resolveImportedArrayOfStructField(TemporaryImportedField importedField, DocumentReference reference,
        ...
        if (targetField.getDataType() instanceof ArrayDataType) {
            ArrayDataType arrayType = (ArrayDataType) targetField.getDataType();
    ...
    private void resolveImportedMapOfStructField(TemporaryImportedField importedField, DocumentReference reference,
                                                 ImmutableSDField targetField, boolean validate) {
        if (targetField.getDataType() instanceof MapDataType) {
            MapDataType mapType = (MapDataType) targetField.getDataType();
            DataType mestedKeyType = mapType.getKeyType();
            DataType nestedValueType = mapType.getValueType();
            if (nestedValueType instanceof StructDataType) {
                resolveImportedNestedField(importedField, reference, targetField.getStructField("key"), true);
